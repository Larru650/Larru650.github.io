...
async function getExactVersion(packageName, requestedVersion) {
  ...
  try {
    const exactVersion = semver.maxSatisfying(
      Object.keys(npmResponse.versions),
      requestedVersion
    );
    return {
      packageName,
...
const getExactVersionMemoized = mem(
  (packageName, requestedVersion) =>
    getExactVersion(packageName, requestedVersion),
  {
...
async function resolveDependencies(packageName, requestedVersion, tree) {
  ...
  const packageNameExactVersion = await getExactVersionMemoized(
    packageName,
    requestedVersion
  );
  const exactVersion = packageNameExactVersion?.exactVersion;
  if (!exactVersion) {
    // unable to determine exact version; bail
    return tree;
  }
  ...
  const combinedDependencies = isTopLevel
  ...
  const exactVersionPromises = combinedDependenciesKeys.reduce(
    (acc, dependencyName) => {
  ...
  const dependencyNameExactVersionMap = exactVersions.reduce(
    (acc, exactVersionResult) => {
  ...
  const resolveDependenciesPromises = combinedDependenciesKeys.reduce(
    (acc, dependencyName) => {
      const dependencyExactVersion =
        dependencyNameExactVersionMap[dependencyName];
      const versionedDependencyName = `${dependencyName}@${dependencyExactVersion}`;
      tree[versionedPackageName][versionedDependencyName] = {};
      const resolvedDependencies = resolveDependenciesMemoized(
        dependencyName,
        dependencyExactVersion,
        tree[versionedPackageName]
      );
...
export const resolveDependenciesMemoized = mem(
  (packageName, requestedVersion, tree) =>
    resolveDependencies(packageName, requestedVersion, tree),
  {
